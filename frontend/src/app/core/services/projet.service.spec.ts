...
describe('ProjetService', () => {
  ...
  it('doit récupérer tous les projets', async () => {
  ...
  it('doit récupérer un projet par ID', async () => {
  ...
  it('doit mettre à jour un projet', async () => {
  ...
  it('doit récupérer les livrables d\'un projet', async () => {
    spyOn(livrableService, 'recupererLivrables').and.callThrough();

    livrableService.recupererLivrables().subscribe();

    const req = httpMock.expectOne(`${environment.apiUrl}/livrables`);
    req.flush([]);

    expect(livrableService.recupererLivrables).toHaveBeenCalled();
  });

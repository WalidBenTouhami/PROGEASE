...
async function checkServerAvailability() {
    try {
        ...
        console.error(
            "❌ Impossible de se connecter au serveur. Assurez-vous que le serveur est en cours d'execution sur",
            baseUrl
        );
        if (error.response) {
    ...
    newman.run(
        ...
        function (err, summary) {
            ...
            if (summary && summary.run && summary.run.stats) {
                ...
                if (failures > 0) {
                    console.log(
                        '⚠️  Des tests ont echoue, consultez le rapport pour plus de details.'
                    );
                } else {

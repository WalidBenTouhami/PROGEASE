...
const utilisateurs = [
    {
        ...
        role: Enum.UtilisateurRole.ETUDIANT,
    },
    {
        nom: 'Trabelsi',
        prenom: 'Sarra',
    ...
    },
    {
        ...
        role: Enum.UtilisateurRole.TUTEUR,
    },
    {
        nom: 'Ben Ayed',
        prenom: 'Leila',
...
async function seed() {
    try {
        ...
        const etudiants = insertedUtilisateurs
            .filter(u => u.role === Enum.UtilisateurRole.ETUDIANT)
            .map(u => u._id);
        const tuteurs = insertedUtilisateurs
            .filter(u => u.role === Enum.UtilisateurRole.TUTEUR)
            .map(u => u._id);
        ...
        insertedProjets.forEach((projet, idx) => {
            livrableTemplates.forEach((template, lidx) => {
                livrablesToInsert.push({
                    ...
                    projetId: projet._id,
                    statut:
                        idx % 2 === 0 ? Enum.StatutLivrable.EN_COURS : Enum.StatutLivrable.TERMINE,
                    creeLe: new Date(),
                    majLe: new Date(),
                });
            });
        ...
        for (const projet of insertedProjets) {
            const livrablesIds = insertedLivrables
                .filter(l => l.projetId.toString() === projet._id.toString())
                .map(l => l._id);

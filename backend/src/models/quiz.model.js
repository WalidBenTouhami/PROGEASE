    ...
    for (const question of this.questions) {
        if (question.type !== 'TEXTE_LIBRE') {
            ...
            if (optionsCorrectes.length === 0) {
                next(
                    new Error(
                        `La question "${question.texte}" doit avoir au moins une option correcte`
                    )
                );
                return;
            }
            if (question.type === 'CHOIX_UNIQUE' && optionsCorrectes.length > 1) {
                next(
                    new Error(
                        `La question à choix unique "${question.texte}" ne peut avoir qu'une seule option correcte`
                    )
                );
                return;
            }

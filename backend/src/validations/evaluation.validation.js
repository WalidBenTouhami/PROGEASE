...
const evaluationSchema = yup.object().shape({
    ...
    criteria: yup.array().of(
        yup.object().shape({
            ...
            comments: yup.string().max(500, 'Criterion comments must not exceed 500 characters'),
        })
    ),
    projetId: yup.string().required('Projet ID is required'),
    evaluatorId: yup.string().required('Evaluator ID is required'),
});
